...
T["normalize_remote unifies url forms"] = function()
  local want = "github.com/shawn/gloss.nvim"
  eq(project.normalize_remote("git@github.com:shawn/gloss.nvim.git"), want)
  eq(project.normalize_remote("https://github.com/shawn/gloss.nvim"), want)
  eq(project.normalize_remote("ssh://git@github.com/shawn/gloss.nvim.git"), want)
...
T["detect finds the project_dir marker upward"] = function()
  ...
  vim.fn.mkdir(vim.fs.joinpath(root, ".gloss"), "p")
...
T["init_in_repo creates the marker and a valid header file"] = function()
  ...
  eq(res.created, true)
  eq(res.migrated, 0)
  eq(vim.fn.readfile(res.path)[1], '{"gloss":1}')
...
T["init_in_repo writes the merge=union gitattribute once"] = function()
  ...
  eq(vim.fn.readfile(ga), { ".gloss/glossary.jsonl merge=union" })
end
...
T["init_in_repo migrates the out-of-repo store, backs it up, retires it"] = function()
  ...
  local res = project.init_in_repo(root)
  eq(res.migrated, 1)
  local migrated = require("gloss.store.jsonl").open(res.path):get("DLQ")
...
T["deinit imports the jsonl into an out-of-repo store"] = function()
  ...
  vim.fn.mkdir(root, "p")
  local res = project.init_in_repo(root)
  require("gloss.store.jsonl").open(res.path):upsert({ term = "API", definition = "d" })

  local out = project.deinit(root)
  eq(out.imported, 1)
  eq(require("gloss.store.sqlite").open(out.path):get("API").definition, "d")
